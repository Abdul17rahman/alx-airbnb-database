EXPLAIN ANALYZE
SELECT * FROM booking INNER JOIN user ON booking.user_id = user.user_id INNER JOIN property ON booking.property_id = property.property_id INNER JOIN payment ON payment.booking_id = booking.booking_id WHERE payment.amount > 100 AND property.location = 'New York';
